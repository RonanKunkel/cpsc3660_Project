...
$stmt = $conn->prepare("
    ...
    WHERE v.year BETWEEN :yearMin AND :yearMax
...
// Display cars
$stmt = $conn->prepare("
    (SELECT v.vin, v.year, v.make, v.model, v.miles, v.book_price, p.price_paid, p.purchase_date
    FROM vehicle AS v
    NATURAL JOIN purchase AS p
    WHERE v.year BETWEEN :yearMin AND :yearMax
      AND v.miles BETWEEN :kmMin AND :kmMax
...
<body>
    ...
    <section class="main-content">
        ...
        <br>
        <div>
            <p>Total Vehicles: <?php echo $carAmount; ?></p>
            <table border="1">
                <tr>
                    <th>VIN</th>
                    <th>Year</th>
